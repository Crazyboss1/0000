...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
            ...
            try:
                if AUTH_CHANNEL and not await is_subscribed(client, query):
                    ...
                    msg = await query.message.reply(
                       ...
                       f'<b>📂 Fɪʟᴇ Nᴀᴍᴇ</b> : <code>[MOVIEHUB] {title}</code>\n\n'              
                      ...
                      'html',
                       reply_markup=InlineKeyboardMarkup(
                           [
                               [
                                   InlineKeyboardButton('📥 𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽 𝖫𝗂𝗇𝗄 📥 ', url = dd.link)
                               ],                       
                               [
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🤖 Updates', url='https://t.me/ItsMoviesPoint')
            ],[
        ...
        buttons= [[
            InlineKeyboardButton('🤖 𝙐𝙥𝙙𝙖𝙩𝙚𝙨', url='https://t.me/ItsMoviesPoint'),
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            if btn is not None:
                try:
                    if fileid == "None":
                        if btn == "[]":
                            Joelkb1 = await client.send_message(group_id, reply_text, disable_web_page_preview=True)
                            await asyncio.sleep(600)
                            await Joelkb1.delete()
                        else:
                            button = eval(btn)
                            Joelkb2 = await client.send_message(
                                group_id,
                                reply_text,
                                disable_web_page_preview=True,
                            ...
                            await asyncio.sleep(600)
                            await Joelkb2.delete()
                    elif btn == "[]":
                        Joelkb3 = await client.send_cached_media(
                            group_id,
                            fileid,
                        ...
                        await asyncio.sleep(600)
                        await Joelkb3.delete()
                    else:
                        button = eval(btn)
                        Joelkb4 = await message.reply_cached_media(
                            fileid,
                            caption=reply_text or "",
                            reply_markup=InlineKeyboardMarkup(button),
                            reply_to_message_id=reply_id
